var backpackDom                           // passed back when messaged
...
function backgroundInit() {
    ...
    chrome.extension.onRequest.addListener(
        function(request, sender, sendResponse) {
            if (request.get == "backpack") {
                sendResponse({doc:backpackDom})
            } else if (request.get == "profileUrl") {
                sendResponse({url:getTF2ItemsUrl()})
            } else {
                sendResponse({})
            }
    })
